...
splitFile <- function(name, out.dir) {
    ...
    # Writing training dataset to the output file.
    outCon <- file(outTraining, open = "w")
    writeLines(data.training, outCon)
    close(outCon)

    # Writing testing dataset to the output file.
    outCon <- file(outTesting, open = "w")
    writeLines(data.testing, outCon)
    close(outCon)

    # Writing validation dataset to the output file.
    outCon <- file(outValidation, open = "w")
    writeLines(data.validation, outCon)
    close(outCon)
}
```
