...
The source code is available on GitHub: https://github.com/serdioa/DataScienceCapstone
...
We add to our lists candidates which are not present it in yet.
...
candidates from the 1-grams table.
...
"can't go anywehre with them" appear only once. Intuitively the first n-gram
...
(`r sprintf("%.2f%%", ngram.1.pct * 100)`) appears just once.
...
If we keep in an n-gram table only entries which appear in the corpus at least
$m$ times and remove the rest, we will say that the table has a *threshold* $m$.
For example, a n-gram table with the threshold 3 contains only n-grams which
appear at least 3 times. Obviously, the complete table with all available
...
samples each. The chart and table below shows how our prediction quality
changes as we increase the threshold.
...
green contains the highest value, that is the best precision. The last line
compares precision with threshold 6 vs. the best precision in the same column.
...
often will assign to it a high score.

* As we increase the threshold, the precision of our prediction is reduced, but
not very dramatically. The average precision loss in our table is 1.5%.
...
As the chart and the table demonstrates, increasing the threshold dramatically
reduces the required memory. The size reduction is less for 1-grams (from
`r stat.size.tbl$Size.1[1]` MiB to `r stat.size.tbl$Size.1[6]` MiB, that is to
`r stat.size.tbl$Size.1.Pct[6]`% of the original), but is pretty spectacular for
5-grams (from `r stat.size.tbl$Size.5[1]` MiB to `r stat.size.tbl$Size.5[6]`
MiB, that is to `r stat.size.tbl$Size.5.Pct[6]`% of the original). The total
required memory goes down from `r stat.size.tbl$Size.All[1]` MiB to
...
kable(stat.predict.time.fmt,
      ...
      booktabs = TRUE,
      col.names = c("Threshold", "Duration, hours", " Duration per sample, ms"),
      align = c("l", "r", "r")) %>%
  kable_styling(bootstrap_options = c("striped", "bordered", "hover"),
                full_width = FALSE)
```
...
average prediction time from `r stat.predict.time.fmt$Processing.Avg.Ms[1]` to
`r stat.predict.time.fmt$Processing.Avg.Ms[6]`, or by
...
out-of-sample data, that is on the validation data set
[we have stored](#download_and_split). Our testing approach is the same as
before: 100 batches of 1.000 samples each for each corpus, as well as for the
aggregated corpora.
