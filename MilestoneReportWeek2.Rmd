...
* [Conclusions and next steps](#conclusions_next_steps)
...
ggplot(data = stat.freq.pct, aes(x = Count, y = Freq.Pct * 100, group = Source)) +
...
in the white-list with the token `UNK`.
...
ggplot(all.2gram.95.freq.pct,
             aes(x = Count, y = Freq.Pct * 100, fill = Type)) +
  scale_x_continuous(trans = "log10",
                     breaks = c(10, 100, 1000, 10000, 100000, 1000000, 10000000),
                     labels = c("10", "100", "1,000", "10,000", "100,000", "1,000,000", "10,000,000")) +
  geom_area() + 
  scale_fill_brewer(palette="Greens", breaks=rev(levels(all.2gram.95.freq.pct$Type))) +
  geom_segment(aes(x = all.2gram.95.freq.30, y = 0, xend = all.2gram.95.freq.30, yend = 50)) +
  geom_segment(aes(x = all.2gram.95.freq.20, y = 0, xend = all.2gram.95.freq.20, yend = 60)) +
  geom_segment(aes(x = all.2gram.95.freq.10, y = 0, xend = all.2gram.95.freq.10, yend = 70)) +
  geom_segment(aes(x = all.2gram.95.freq.5, y = 0, xend = all.2gram.95.freq.5, yend = 80)) +
  geom_segment(aes(x = all.2gram.95.freq.1, y = 0, xend = all.2gram.95.freq.1, yend = 90)) +
  annotate(geom="text", x=all.2gram.95.freq.30, y=52, label="30")+
  annotate(geom="text", x=all.2gram.95.freq.20, y=62, label="20")+
  annotate(geom="text", x=all.2gram.95.freq.10, y=72, label="10")+
  annotate(geom="text", x=all.2gram.95.freq.5, y=82, label="5")+
  annotate(geom="text", x=all.2gram.95.freq.1, y=92, label="1")+
  labs(title = "Cumulative Frequency of Bigrams") +
  labs(x = "Unique bigrams") +
  labs(y = "Cumulative Frequency, %") +
  theme_bw(base_size = 14) 
...
Charts below show coverage of corpora by pruned 2- to 6-grams, where
different color indicates n-grams with a different number of pruned words (`UNK`
tokens). The same as for 2-grams, charts also show for several number points
where n-grams were encountered a particular number of times.
...
ggplot(all.3gram.95.freq.pct,
  ...
  geom_segment(aes(x = all.3gram.95.freq.30, y = 0, xend = all.3gram.95.freq.30, yend = 50)) +
  geom_segment(aes(x = all.3gram.95.freq.20, y = 0, xend = all.3gram.95.freq.20, yend = 60)) +
  geom_segment(aes(x = all.3gram.95.freq.10, y = 0, xend = all.3gram.95.freq.10, yend = 70)) +
  geom_segment(aes(x = all.3gram.95.freq.5, y = 0, xend = all.3gram.95.freq.5, yend = 80)) +
  geom_segment(aes(x = all.3gram.95.freq.1, y = 0, xend = all.3gram.95.freq.1, yend = 90)) +
  annotate(geom="text", x=all.3gram.95.freq.30, y=52, label="30")+
  annotate(geom="text", x=all.3gram.95.freq.20, y=62, label="20")+
  annotate(geom="text", x=all.3gram.95.freq.10, y=72, label="10")+
  annotate(geom="text", x=all.3gram.95.freq.5, y=82, label="5")+
  annotate(geom="text", x=all.3gram.95.freq.1, y=92, label="1")+
...
ggplot(all.4gram.95.freq.pct,
  ...
  geom_segment(aes(x = all.4gram.95.freq.30, y = 0, xend = all.4gram.95.freq.30, yend = 50)) +
  geom_segment(aes(x = all.4gram.95.freq.20, y = 0, xend = all.4gram.95.freq.20, yend = 60)) +
  geom_segment(aes(x = all.4gram.95.freq.10, y = 0, xend = all.4gram.95.freq.10, yend = 70)) +
  geom_segment(aes(x = all.4gram.95.freq.5, y = 0, xend = all.4gram.95.freq.5, yend = 80)) +
  geom_segment(aes(x = all.4gram.95.freq.1, y = 0, xend = all.4gram.95.freq.1, yend = 90)) +
  annotate(geom="text", x=all.4gram.95.freq.30, y=52, label="30")+
  annotate(geom="text", x=all.4gram.95.freq.20, y=62, label="20")+
  annotate(geom="text", x=all.4gram.95.freq.10, y=72, label="10")+
  annotate(geom="text", x=all.4gram.95.freq.5, y=82, label="5")+
  annotate(geom="text", x=all.4gram.95.freq.1, y=92, label="1")+
...
ggplot(all.5gram.95.freq.pct,
  ...
  geom_segment(aes(x = all.5gram.95.freq.30, y = 0, xend = all.5gram.95.freq.30, yend = 50)) +
  geom_segment(aes(x = all.5gram.95.freq.20, y = 0, xend = all.5gram.95.freq.20, yend = 60)) +
  geom_segment(aes(x = all.5gram.95.freq.10, y = 0, xend = all.5gram.95.freq.10, yend = 70)) +
  geom_segment(aes(x = all.5gram.95.freq.5, y = 0, xend = all.5gram.95.freq.5, yend = 80)) +
  geom_segment(aes(x = all.5gram.95.freq.1, y = 0, xend = all.5gram.95.freq.1, yend = 90)) +
  annotate(geom="text", x=all.5gram.95.freq.30, y=52, label="30")+
  annotate(geom="text", x=all.5gram.95.freq.20, y=62, label="20")+
  annotate(geom="text", x=all.5gram.95.freq.10, y=72, label="10")+
  annotate(geom="text", x=all.5gram.95.freq.5, y=82, label="5")+
  annotate(geom="text", x=all.5gram.95.freq.1, y=92, label="1")+
...
ggplot(all.6gram.95.freq.pct,
  ...
  geom_segment(aes(x = all.6gram.95.freq.30, y = 0, xend = all.6gram.95.freq.30, yend = 50)) +
  geom_segment(aes(x = all.6gram.95.freq.20, y = 0, xend = all.6gram.95.freq.20, yend = 60)) +
  geom_segment(aes(x = all.6gram.95.freq.10, y = 0, xend = all.6gram.95.freq.10, yend = 70)) +
  geom_segment(aes(x = all.6gram.95.freq.5, y = 0, xend = all.6gram.95.freq.5, yend = 80)) +
  geom_segment(aes(x = all.6gram.95.freq.1, y = 0, xend = all.6gram.95.freq.1, yend = 90)) +
  annotate(geom="text", x=all.6gram.95.freq.30, y=52, label="30")+
  annotate(geom="text", x=all.6gram.95.freq.20, y=62, label="20")+
  annotate(geom="text", x=all.6gram.95.freq.10, y=72, label="10")+
  annotate(geom="text", x=all.6gram.95.freq.5, y=82, label="5")+
  annotate(geom="text", x=all.6gram.95.freq.1, y=92, label="1")+
...
As $n$ grows, the number of repeated n-grams decreases. This property is quite
obvious: for example, there are much more common 2-grams (like "last year" or
"good luck") as common 6-grams. A consequence of this property is less obvious, but is clearly visible on
charts: as $n$ grows, one require more and more unique n-grams to cover the same
percentage of the text. For single words we could choose a small subsets that
covers 95% of the corpora, but for n-grams achieving a high corpora coverage
with a small subset is impossible.

```{r cover123456grams.count.tbl, echo=FALSE, cache=TRUE}
all.ngram.freq.tbl <-
    data.frame(
...
kable(all.ngram.freq.tbl, booktabs = TRUE,
      col.names = c("Corpora Coverage",
                    "2-grams",
                    "3-grams",
                    "4-grams",
...
The table above shows a percentage of n-grams required to cover a particular
percentage of the aggregated corpus for various n. For example, one require
`r all.ngram.freq.tbl[2,2]`% of 2-grams to cover 50% of the corpus, but the same
coverage requires `r all.ngram.freq.tbl[2,3]`% of 3-grams. As we could see,
even for 2-grams we can't significantly reduce a number of unique n-grams
without significantly reducing the coverage as well.
  
# <a name="conclusions_next_steps"></a>Conclusions and next steps

Conclusions from the data analysis:

* If we keep only most often used words required for 95% coverage of the
corpora, we may significatly reduce the number of distinct words, but the number
of 2- to 6-grams is not significantly affected.
* To get a decent coverage of the corpora by 2- to 6-grams, we require millions
of entries. To reduce memory usage, probably we have to use some encoding
shema. Here we may again return to the idea of keeping only most often used
words to reduce the size of the encoded data.
* For small $n$, many n-grams are encountered in the corpora multiple times,
but for large $n$ most n-grams are encountered just once. This is intuitively
obvious: we expect common bigrams "last year" or "good luck" to be repeated
many time, but a probability that a particular 6-gram repeats is pretty low.
When developing a prediction model, we have to test if it makes sense at all
to include in the model n-grams with large $n$, since this could be an
overfitting to the training set without any benefits for the prediction quality.

Open questions:

* Should we include stop-words in our prediction set? On one hand, stop-words
are too common and may be considered a syntactical "garbage", on the other
hand they are an important part of a natural language.
* How should we use the stemming? On one hand, stemming may reduce the number
of n-grams and improve prediction quality. On the other hand, we want to predict
full words, not just stems. At the moment we are inclined to stem words used
for the prediction, but not the predicted word. This approach may require
a custom implementation of the tokenization algorithm.
* Should we replace seldom words with the special token `UNK` or should we keep
such words?
* Does it make sense to use n-grams for large $n$, or could we overfit our
model with them?
* Should we replace common abbreviations with full text before training our
algorithm, or should we keep the abbreviations (for example, AFAIK = "as far as
I know")?
* Should we remove profanity from the corpora, or should we keep it as a part
of the natural language? At the moment we are inclined to keep profanity in the
dictionary, but never predict it. Keeping profanity in the dictionary may
improve quality of prediction for non-profane words. For example, after the
bigram "for fuck's" we may predict the non-profane word "sake". If we would have
excluded profanity from our dictionary, we may miss the right prediction in this
case.

To answer most questions above, we have to create several models and run them
against a test data set.

Next steps:

* Implement the simplest possible prediction algorithm.
* Test the algorithm on the test data set, analyze quality of prediction
