...
(DFM) for words in documents, and aggregates it over all documents to a Feature
Vector. The second helper function enriches the Feature Vector with additional
values useful for our analysis, such as cumulated coverage of text. We will
use the second function later as well, to enrich data on n-grams.

```{r}
# Calculate Document Feature Matrix (DFM) for words in documents, and aggregate
# it over all documents to a Feature Vector.
build.1gram.fv <- function(text.corpus) {
    text.dfm <- dfm(text.corpus)
    colSums(text.dfm)
}
...
```{r}
blogs.freq <- enrich.fv(build.1gram.fv(blogs.corpus))
news.freq <- enrich.fv(build.1gram.fv(news.corpus))
twitter.freq <- enrich.fv(build.1gram.fv(twitter.corpus))
all.freq <- enrich.fv(build.1gram.fv(blogs.corpus + news.corpus + twitter.corpus))
```
