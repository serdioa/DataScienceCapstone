...
#
# Splits one file on traning, testing and validation subsets.
#
# @param name - the file to split
...
#
# Splits all corpora files on traning, testing and validation subsets.
# The corpora files are expected to be located in the directory "cache".
# The output files are written into the same directory.
#
split.all <- function() {
    if (!file.exists(file.path("cache", "en_US.blogs.training.txt"))) {
        message("Splitting blogs")
        split.file(file.path("cache", "final", "en_US", "en_US.blogs.txt"), "cache")
        message("Done splitting blogs")
    }
    if (!file.exists(file.path("cache", "en_US.news.training.txt"))) {
        message("Splitting news")
        split.file(file.path("cache", "final", "en_US", "en_US.news.txt"), "cache")
        message("Done splitting news")
    }
    if (!file.exists(file.path("cache", "en_US.twitter.training.txt"))) {
        message("Splitting twitter")
        split.file(file.path("cache", "final", "en_US", "en_US.twitter.txt"), "cache")
